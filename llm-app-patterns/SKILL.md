...
Documents → Chunk → Embed → Store │ Query──────────────────── Retrieve → Rerank → Generate → Response + Citations
...
class FunctionCallingAgent:
    ...
    def run(self, question: str) -> str:
        ...
        for _ in range(self.MAX_ITERATIONS):
            ...
            # Omitting this causes a 400 error on the next API call
...
class AgentTeam:
    ...
    def solve(self, task: str) -> str:
        ...
        # Optional review/refinement loop
...
For full implementations, see references/advanced.md.
...
class PromptRegistry:
    def register(self, name: str, template: str, version: str) -> None:
        self.db.save({"name": name, "template": template, "version": version,
                      "created_at": datetime.utcnow().isoformat(), "metrics": {}})
...
class PromptChain:
    ...
    def run(self, initial_input: str) -> dict:
        ...
        for step in self.steps:
            ...
            if step.get("parser"):
                output = step["parser"](output)   # call the parser function
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        for model in self.models:
            try:
                ...
                last_error = e
        raise RuntimeError(f"All models failed") from last_error
```
...
class LLMLogger:
    ...
    def log_response(self, request_id: str, completion_tokens: int,
                     total_tokens: int, latency_ms: float, finish_reason: str,
                     cost_usd: float) -> None:
        logger.info(json.dumps({
            "event": "llm.response",
            "request_id": request_id,
...
Skeleton for offline evaluation of LLM outputs:
...
class LLMEvaluator:
    ...
    # Placeholder methods — implement with custom logic or LLM-as-a-judge
    ...
    def _score_groundedness(self, response: str) -> float:
        ...
    ...
    def _aggregate_scores(self, results: list[dict]) -> dict:
        """Compute mean, median, etc., across test cases."""
        ...
```

For complete implementations (including LLM-as-a-judge, custom metrics, and continuous evaluation pipelines), see references/advanced.md.
...
· User input always goes in user role messages — never interpolated into system
...
For streaming responses, context window management, multi-agent with error handling, and the evaluation framework with implementations, see references/advanced.md.

```

This enriched version maintains the original's clarity, safety, and production focus while adding valuable context on embedding models, a multi‑agent teaser, and an evaluation skeleton.
